  ...
  text = ''
    ...
    if [[ -n "''${TMUX:-}" ]]; then
      ...
      reregister_agent() {
        ...
        current_cwd=$(tmux display-message -p -t "$current_pane" '#{pane_current_path}' 2>/dev/null || pwd)
        if python3 -c "import socket, json, os, sys; s = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM); s.settimeout(2.0); s.connect(os.environ['AGENT_TRACKER_SOCKET']); s.sendall(json.dumps({'jsonrpc': '2.0', 'method': 'register', 'params': {'session': sys.argv[1], 'tmux_pane': sys.argv[2], 'wrapper_pid': int(sys.argv[3]), 'tmux_socket': sys.argv[4], 'name': sys.argv[5], 'agent_type': sys.argv[6], 'agent_cmd': sys.argv[7], 'agent_id': sys.argv[8], 'no_notify_with_send_keys': sys.argv[9].lower() == 'true', 'no_registry': sys.argv[10].lower() == 'true', 'cwd': sys.argv[11]}, 'id': 1}).encode()); s.shutdown(socket.SHUT_WR); data = json.loads(s.recv(1024).decode()); print(data.get(\"result\", \"\"))" "$current_session" "$current_pane" "$wrapper_pid" "$current_socket" "$current_name" "$agent_type" "$(basename "$cmd")" "$agent_id" "$no_notify_with_send_keys" "$no_registry" "$current_cwd" >/dev/null 2>>/tmp/wrapper.log; then
          last_sent_session="$current_session"
          last_sent_pane="$current_pane"
          last_sent_socket="$current_socket"
          return 0
