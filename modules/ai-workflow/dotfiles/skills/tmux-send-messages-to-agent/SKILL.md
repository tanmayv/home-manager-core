...
2. **Identity**: Once named, change your tmux pane title to reflect your name:
   `tmux select-pane -T "Agent:<name>"`
   If your pane is the only one in the window, also change the window title:
...
5. **Targeting**: To find other agents to communicate with, query tmux panes across all sessions:
   `tmux list-panes -a -F "#{pane_id} #{pane_title} #{window_name}"`
   Look for panes or windows starting with `Agent:`.
6. **Protocol**:
