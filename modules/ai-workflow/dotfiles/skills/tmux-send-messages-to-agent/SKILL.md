...
2. **Identity**: Once named, set your agent name using the custom tmux option `@agent_name`. This will be displayed in the pane border, which is formatted as `─( [Pane ID] | [Agent Name] | [Title] )─`:
...
5. **Targeting**: To find other agents to communicate with, query tmux panes across all sessions. You can easily see an agent's Pane ID (e.g., `%4`) and Name in their pane border. To list all agents programmatically:
   `tmux list-panes -a -F "#{pane_id} #{@agent_name} #{pane_title} #{window_name}" | grep -v "no-name"`
   Look for panes where `@agent_name` matches the agent you are looking for.
6. **Protocol**:
