...
3.  **Create Entity**:
    ...
    *   If **Skill**: Create a new `SKILL.md` file in `~/.gemini/skills/{skill_name}/` with appropriate YAML frontmatter and instructions.
    *   If **Agent**: Create a new directory in `~/.gemini/agents/{agent_name}/` with an `agent.json` file and optionally a `config.yaml` or `GEMINI.md`.
